import asyncio , aiocron , telethon , logging , time , random , os , html
...
async def _(event):
    ...
    try:
        ...
        location = str(e)
    caption = """<b>Extracted by Px_Sorce<b>
...
async def bioclock(event):
    ...
    while True:
        ...
        bio = f"پروکسی پرسرعت | Fast proxy | @mtproxy_pain {DMY}"
        logger.info(bio)
...
@client.on(events.NewMessage(pattern=r"painhelper"))
async def help(event):
    if event.fwd_from:
        return
    await event.edit("𝗨𝘀𝗲𝗿 𝗺𝗲𝗻𝘂 𝗼𝗽𝗲𝗻𝗲𝗱⤵️")
    await event.reply('''pain عزیز به منوی راهنما خوش آمدید
➖➖➖➖➖➖➖➖➖➖
.clone
...
جهت تنظیم ساعت در بیو⏰
➖➖➖➖➖➖➖➖➖➖
.timebio on/off

جهت تنظیم ساعت کنار اسم⌚
.jio 
